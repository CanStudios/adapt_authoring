...
OutputPlugin.prototype.applyTheme = function(tenantId, courseId, jsonObject, destinationFolder, next) {
  ...
  var themeVariables = jsonObject['course'][0].hasOwnProperty('themeVariables')
    ? jsonObject['course'][0].themeVariables
    : false;
  ...
  // Check if the theme selected has customisations or if custom styling has been applied
  if (themeVariables || customStyle) {
            ...
            if (themeVariables) {
              ...
              async.series([
                ...
                function(seriesCallback) {
                  ...
                  async.eachSeries(_.keys(themeVariables), function(key, innerCallback) {
                    if (key !== '_type') {
                      if (typeof themeVariables[key] === 'object') {
                        // Iterate the properies and add them to the array
                        async.each(_.keys(themeVariables[key]), function(childKey, secondInnerCallback) {
                ...
                function(seriesCallback) {
                  fs.writeFile(path.join(destinationFolder, Constants.Folders.Less, Constants.Filenames.ThemeVariables), modifiedProperties, 'utf8', function (err) {
                    if (err) {
